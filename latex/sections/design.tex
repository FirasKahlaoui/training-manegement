...
\subsection{Authentication Process (Login)}
This diagram details the exchange between the frontend, the AuthController, the AuthService, and the database during the user authentication process.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Figures/Sequence_login.png}
    \caption{Sequence Diagram: User Authentication}
    \label{fig_seq_login}
\end{figure}

\subsection{Managing Training Sessions (Creation)}
The following diagram illustrates the workflow for creating a new training session, showing the data validation and persistence layers.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Figures/Create Formation.png}
    \caption{Sequence Diagram: Adding a New Formation}
    \label{fig_seq_add_form}
\end{figure}
